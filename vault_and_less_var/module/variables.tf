...
variable "site_type" {
  description = "Site type for future module behavior."
  type        = string

  validation {
    condition     = contains(["battery", "battery-solar", "battery-wind", "solar", "wind"], var.site_type)
    error_message = "site_type must be one of: battery, battery-solar, battery-wind, solar, wind."
  }
}
