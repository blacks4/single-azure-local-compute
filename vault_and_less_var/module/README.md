...
- **Large VM batches may need lower Terraform parallelism** — Azure Local/MOC operations can fail when many VM creates run concurrently. Set HCP Terraform `TFE_PARALLELISM=1`, or use `TF_CLI_ARGS_plan="-parallelism=1"` and `TF_CLI_ARGS_apply="-parallelism=1"` for agent/local runs.
